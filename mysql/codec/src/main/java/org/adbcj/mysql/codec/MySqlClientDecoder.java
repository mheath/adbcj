...
public class MySqlClientDecoder {
    ...
    /**
     * Holds the remaining number of field packets expected to build the result
     * set
     */
     ...
     * @return the decode message, null if the {@code block} is {@code} false
     * and there is not enough data available to decode the message without
     * blocking
    ...
    protected ServerPacket doDecode(InputStream input, boolean block) throws IOException {
        ...
        if (!block) {
            // Make sure we have enough data for the packet length and the
            // packet number
        ...
        try {
            ...
            switch (state) {
                ...
                case RESPONSE:
                    ...
                    // Get the number of fields. The largest this can be is a
                    // 24-bit
                ...
                case ROW:
                    ...
                    for (int i = 0; i < fields.length;) {
                        ...
                        if (fieldCount != IoUtils.NULL_VALUE) {
                            // We will have to move this as some datatypes will
                            // not be sent across the wire as strings
                            ...
                            switch (field.getColumnType()) {
                                ...
                                case DATE:
                                    ...
                                    break;
                                case TIMESTAMP:
                                    value = Timestamp.valueOf(strVal);
                                    break;
                                case FLOAT:
    ...
    protected OkResponse decodeOkResponse(BoundedInputStream in, int length, int packetNumber) throws IOException {
        ...
        return new OkResponse(length, packetNumber, affectedRows, insertId, serverStatus, warningCount, message);
    }
    ...
    protected ResultSetFieldResponse decodeFieldResponse(InputStream in, int packetLength, int packetNumber)
                                                                                                            throws IOException {
        ...
        MysqlField field = new MysqlField(fieldIndex,
            catalogName,
            schemaName,
            tableLabel,
            tableName,
    ...
    // TODO: This stream implementation doesn't even work b ecause it doesn't
    // delegate all InputStream methods
